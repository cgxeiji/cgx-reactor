...
- **Passable engine handle** — The engine's full template type (`engine<Config, Clock, Logger, Entries...>`) is not nameable until `make_engine` instantiates it, so a reactor task cannot take the engine by typed reference to call engine APIs (e.g. `dump()`, `trigger()`, `done()`). Any task that needs the engine falls back to a free template coroutine (`template<typename E> task bootstrap(E& eng, ...)`) resumed manually via `handle().resume()`, whose frame is **heap-allocated** and never reclaimed — an anti-pattern. This blocks registering such orchestrator tasks (e.g. `bootstrap`) as scratchpad/reserved engine-managed tasks, because (a) the member-task signature would need the engine type which doesn't exist until after the class is defined (circular dependency), and (b) a free task's signature needs the engine type which itself contains `free_spec<&that_task>` (circular). Expose a non-template type-erased `engine_handle` (vtable or fn-pointer dispatch exposing `trigger`/`try_trigger`/`done`/`dump`) that can be passed into tasks and other MCU code by value. Surfaced while writing the RP2040 example's periodic `dump()` task (worked around with `void(*)(void*)` + ctx) and its `bootstrap` orchestrator coroutine.
- **Scratchpad parameter forwarding** — Scratchpad tasks currently take **no parameters**: `trigger<Fn>(args...)` discards `args` for scratchpad entries (returns `scratchpad_trigger_awaiter{this, idx, {}, {}}` with no arg storage), and `execute_scratchpad_at` invokes `fn()` / `(obj->*fn)()` with zero args. Reserved tasks forward args via `trigger_reserved<I>(args...)`. Add arg forwarding to the scratchpad awaiter + `execute_scratchpad_at` so one-shot scratchpad tasks can be parameterized (needed for the passable-engine-handle vision where a scratchpad `bootstrap` takes `engine_handle`).
- **Free-function scratchpad support** — `spec_unfolder<free_spec<Fn>>` hardcodes `task_descriptor<Fn, void, false>` (`is_scratchpad=false`), so free functions are **always reserved** and can never be scratchpad. `scratch_v` only applies to member entries inside `bound`. Either extend `register_task` to accept a scratch variant (`register_scratch_task<Fn>()` / `scratch_v<&free_fn>`) or document the member-only constraint.
- **Scratchpad example `schedule()` heap frame** — `examples/scratchpad/main.cpp`'s `schedule()` orchestrator coroutine is a free template task resumed manually, so its frame is heap-allocated and never destroyed — the same anti-pattern as the RP2040 example's `bootstrap`. Fix (together with the passable-engine-handle work) by making the orchestrator an engine-managed task, or explicitly destroying the handle after completion as an interim measure.
