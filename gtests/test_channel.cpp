...
TEST(ChannelTest, BufferFillAndDrain) {
    ...
    auto h = eng.template trigger<&push_many<3>>(ch, 10, 3, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);
    for (int i = 0; i < 3; ++i)
        EXPECT_EQ(push_ec[i], cr::error::ok) << "i=" << i;

    // Drain — all 3 should be available.
    h = eng.template trigger<&pop_many<3>>(ch, 3, pop_vals);
    ASSERT_EQ(h.error(), cr::error::ok);
    for (int i = 0; i < 3; ++i) {
...
TEST(ChannelTest, BlockingPushResolvedByPop) {
    ...
    auto h = eng.template trigger<&push_one<1>>(ch, 10, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);
    EXPECT_EQ(push_ec, cr::error::ok);

    // Push again — buffer full, producer suspends.
    h = eng.template trigger<&push_one<1>>(ch, 20, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);

    // Pop — takes 10, then the producer's 20 moves into the buffer and
    // the producer is resumed (push_ec set to ok).
    h = eng.template trigger<&pop_one<1>>(ch, pop_val);
    ASSERT_EQ(h.error(), cr::error::ok);
    ASSERT_TRUE(pop_val.has_value());
...
TEST(ChannelTest, BlockingPopResolvedByPush) {
    ...
    auto h = eng.template trigger<&pop_one<1>>(ch, pop_val);
    ASSERT_EQ(h.error(), cr::error::ok);
    EXPECT_FALSE(pop_val.has_value());

    // Push — the consumer is waiting, so the value is handed off directly.
    h = eng.template trigger<&push_one<1>>(ch, 99, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);
    EXPECT_EQ(push_ec, cr::error::ok);
    ASSERT_TRUE(pop_val.has_value());
...
TEST(ChannelTest, CloseWakesProducer) {
    ...
    auto h = eng.template trigger<&push_one<1>>(ch, 2, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);
...
TEST(ChannelTest, PingPong) {
    ...
    auto h = eng.template trigger<&ping_pong_consumer<2>>(ch, cons_val);
    ASSERT_EQ(h.error(), cr::error::ok);

    // Trigger producer: pushes 1 (handoff to consumer), then pops
    // (receives 2 from consumer).
    h = eng.template trigger<&ping_pong_producer<2>>(ch, prod_val);
    ASSERT_EQ(h.error(), cr::error::ok);
...
TEST(ChannelTest, PopOnClosedChannel) {
    ...
    auto h = eng.template trigger<&pop_on_closed_channel<4>>(ch, result);
    ASSERT_EQ(h.error(), cr::error::ok);
...
TEST(ChannelTest, PushOnClosedChannel) {
    ...
    auto h = eng.template trigger<&push_on_closed_simple<4>>(ch, push_ec);
    ASSERT_EQ(h.error(), cr::error::ok);
    EXPECT_EQ(push_ec, cr::error::closed);
}
...
TEST(ChannelTest, MultipleProducersFIFO) {
    ...
    ASSERT_EQ(eng.template trigger<&push_a>(ch, 10, ec1).error(), cr::error::ok);
    ASSERT_EQ(eng.template trigger<&push_b>(ch, 20, ec2).error(), cr::error::ok);
    ASSERT_EQ(eng.template trigger<&push_c>(ch, 30, ec3).error(), cr::error::ok);
    ...
    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v1).error(), cr::error::ok);
    ASSERT_TRUE(v1.has_value());
    EXPECT_EQ(*v1, 0);
    EXPECT_EQ(ec1, cr::error::ok);  // push_a succeeded

    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v2).error(), cr::error::ok);
    ASSERT_TRUE(v2.has_value());
    EXPECT_EQ(*v2, 1);
    EXPECT_EQ(ec2, cr::error::ok);  // push_b succeeded

    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v3).error(), cr::error::ok);
    ASSERT_TRUE(v3.has_value());
    EXPECT_EQ(*v3, 2);
    EXPECT_EQ(ec3, cr::error::ok);  // push_c succeeded

    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v4).error(), cr::error::ok);
    ASSERT_TRUE(v4.has_value());
    EXPECT_EQ(*v4, 3);

    // Pops 5-7: drain the three producer values now in the buffer.
    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v5).error(), cr::error::ok);
    ASSERT_TRUE(v5.has_value());
    EXPECT_EQ(*v5, 10);

    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v6).error(), cr::error::ok);
    ASSERT_TRUE(v6.has_value());
    EXPECT_EQ(*v6, 20);

    ASSERT_EQ(eng.template trigger<&pop_one<4>>(ch, v7).error(), cr::error::ok);
...
TEST(ChannelTest, MultipleConsumersFIFO) {
    ...
    ASSERT_EQ(eng.template trigger<&cons_a>(ch, v1).error(), cr::error::ok);
    ASSERT_EQ(eng.template trigger<&cons_b>(ch, v2).error(), cr::error::ok);
    ASSERT_EQ(eng.template trigger<&cons_c>(ch, v3).error(), cr::error::ok);

    // Push values — each wakes the next consumer in FIFO order.
    ASSERT_EQ(eng.template trigger<&push_a>(ch, 100, ec1).error(), cr::error::ok);
    ASSERT_TRUE(v1.has_value());
    EXPECT_EQ(*v1, 100);

    ASSERT_EQ(eng.template trigger<&push_b>(ch, 200, ec2).error(), cr::error::ok);
    ASSERT_TRUE(v2.has_value());
    EXPECT_EQ(*v2, 200);

    ASSERT_EQ(eng.template trigger<&push_c>(ch, 300, ec3).error(), cr::error::ok);
    ASSERT_TRUE(v3.has_value());
    EXPECT_EQ(*v3, 300);
}
...
TEST(ChannelTest, ConstChannelCanPop) {
    ...
    ASSERT_EQ(eng.template trigger<&push_one<4>>(ch, 42, ec).error(), cr::error::ok);
    ASSERT_EQ(ec, cr::error::ok);

    // Pop through a const reference — verifies pop() is const-qualified.
    const auto& cref = ch;
    ASSERT_EQ(eng.template trigger<&pop_one_const<4>>(cref, val).error(),
              cr::error::ok);
